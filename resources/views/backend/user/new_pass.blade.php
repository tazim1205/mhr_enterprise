				...
				<div class="col-sm-10 col-md-8 col-lg-6 col-xl-5 mx-auto d-table h-100">
					<div class="d-table-cell align-middle">
						...
						<div class="card">
							<div class="card-body">
								<div class="m-sm-3">
									<form method="post" action="{{route('user.submit_pass')}}" enctype="multipart/form-data">
                                        @csrf
                                        <input type="hidden" name="email" value="{{$email}}">
										<div class="mb-3">
											<label class="form-label">@lang('user.password') (@lang('user.eight_digit'))</label>
											<input class="form-control form-control-lg @error('password')is-invalid @enderror" type="text" name="password" placeholder=""  value="" required>
                                            @error('password')
                                            <div class="alert alert-danger">{{$message}}</div>
                                            @enderror
										</div>
										<div class="mb-3">
											<label class="form-label">@lang('user.confirm_password') (@lang('user.eight_digit'))</label>
											<input class="form-control form-control-lg @error('confirm_password')is-invalid @enderror" type="text" name="confirm_password" placeholder="" required>
                                            @error('confirm_password')
                                            <div class="alert alert-danger">{{$message}}</div>
                                            @enderror
										</div>
										<div class="d-grid gap-2 mt-3">
											<button type="submit" class='btn btn-lg btn-primary'>@lang('user.change_pass')</button>
                                            <br>
										</div>
									</form>
								</div>
